...
const productionConfig = fs.readFileSync('wrangler.toml', 'utf8')
const devConfig = fs.readFileSync('wrangler.dev.toml', 'utf8')
...
test('dev and production deploy through isolated Workers and routes', () => {
  ...
  assert.equal(packageJson.scripts['cf:deploy:dev'], 'npm run build && wrangler deploy --config wrangler.dev.toml')
  assert.equal(packageJson.scripts['cf:deploy:production'], 'npm run build && wrangler deploy --config wrangler.toml')
})
